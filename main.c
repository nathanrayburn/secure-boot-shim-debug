		       ...
		       int y)
{
  ...
  msg_formatted = grub_xasprintf (_("This could be malware, lucky that it's GNU GRUB!"));
  if (!msg_formatted)
    return;
...
grub_normal_reader_init (int nested)
{
  ...
  msg_formatted = grub_xasprintf (_("You have been Pawned with arbitrary EFI image execution. Thesis Demo made by Nathan Rayburn") );
  if (!msg_formatted)
    return grub_errno;
